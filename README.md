...
| [**total-recall**](https://github.com/88plug/total-recall)&nbsp;`v0.7.1` | Cross-session, cross-CLI memory. Mines transcripts from 8 CLI clients; surfaces operator identity, standing decisions, bans, goals, and past corrections so the model stops re-asking. Zero host-python prereq — bootstraps its own `uv` + python in `$CLAUDE_PLUGIN_DATA` on first run | `4 hooks` · `2 skills` · `15 commands` · `23 MCP tools` | `/plugin install total-recall@88plug` |
